-- Fix booking trigger to work with firebase_uid schema (no UUID id column in users)
-- Migration: 20240101000009_fix_booking_trigger_firebase_uid.sql
-- This fixes the trigger to work after migration 20240101000004 which changed users to use firebase_uid as PK

-- Update the trigger function to use firebase_uid directly instead of UUID id
...
BEGIN
  -- Check if users exist (firebase_uid is now the primary key, no id column)
  SELECT EXISTS(SELECT 1 FROM users WHERE firebase_uid = NEW.customer_id) INTO v_customer_exists;
  SELECT EXISTS(SELECT 1 FROM users WHERE firebase_uid = NEW.professional_id) INTO v_professional_exists;
  ...
  IF NEW.chat_room_id IS NULL AND v_customer_exists AND v_professional_exists THEN
    ...
    IF v_chat_room_id IS NULL THEN
      ...
      INSERT INTO chat_rooms (
        ...
        v_chat_room_id,
        NEW.id,
        NEW.customer_id,  -- Use firebase_uid directly (VARCHAR)
        NEW.professional_id,  -- Use firebase_uid directly (VARCHAR)
        TRUE,
        CURRENT_TIMESTAMP,
        CURRENT_TIMESTAMP
      );
      
      -- Update booking with chat_room_id (UUID)
      UPDATE bookings 
  ...
  IF v_customer_exists AND v_professional_exists THEN
...
'Automatically populates related tables when a booking is created: chat_rooms, invoices, professional_balances, and notifications. Updated to work with firebase_uid schema (no UUID id column in users table). Uses firebase_uid directly for all foreign key references.';
