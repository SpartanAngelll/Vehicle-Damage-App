-- Fix booking trigger to correctly set chat_room_id (UUID) without casting to TEXT
-- Migration: 20240101000008_fix_booking_trigger_chat_room_id.sql

-- Update the trigger function to fix the chat_room_id assignment
...
BEGIN
  ...
  IF NEW.chat_room_id IS NULL AND v_customer_uuid IS NOT NULL AND v_professional_uuid IS NOT NULL THEN
    ...
    IF v_chat_room_id IS NULL THEN
      ...
      -- Update booking with chat_room_id (FIXED: removed ::TEXT cast, use UUID directly)
      UPDATE bookings 
...
'Automatically populates related tables when a booking is created: chat_rooms, invoices, professional_balances, and notifications. Fixed: chat_room_id assignment now uses UUID directly instead of casting to TEXT.';
