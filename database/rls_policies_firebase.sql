...
-- Note: Using public schema for better compatibility with Third Party Auth
...
CREATE POLICY "Customers can manage own job requests"
  ON job_requests FOR ALL
  USING (customer_id = public.firebase_uid())
  WITH CHECK (customer_id = public.firebase_uid());

DROP POLICY IF EXISTS "Professionals can view active job requests" ON job_requests;
CREATE POLICY "Professionals can view active job requests"
...
CREATE POLICY "Professionals can manage own estimates"
  ON estimates FOR ALL
  USING (professional_id = public.firebase_uid())
  WITH CHECK (professional_id = public.firebase_uid());

DROP POLICY IF EXISTS "Customers can view estimates for their requests" ON estimates;
CREATE POLICY "Customers can view estimates for their requests"
  ON estimates FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM job_requests 
      WHERE job_requests.id = estimates.job_request_id 
      AND job_requests.customer_id = public.firebase_uid()
    )
  );
...
CREATE POLICY "Users can view own bookings"
  ...
  USING (
    customer_id = public.firebase_uid() 
    OR professional_id = public.firebase_uid()
  );
...
CREATE POLICY "Users can view own payment records"
  ...
  USING (
    EXISTS (
      ...
      AND (
        bookings.customer_id = public.firebase_uid() 
        OR bookings.professional_id = public.firebase_uid()
      )
    )
  );
...
CREATE POLICY "Professionals can manage own confirmations"
...
CREATE POLICY "Customers can view confirmations for own bookings"
  ON payment_confirmations FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM bookings 
      WHERE bookings.id = payment_confirmations.booking_id 
      AND bookings.customer_id = public.firebase_uid()
    )
  );

-- Professional balances policies
DROP POLICY IF EXISTS "Professionals can view own balance" ON professional_balances;
CREATE POLICY "Professionals can view own balance"
  ON professional_balances FOR SELECT
  USING (professional_id = public.firebase_uid());

DROP POLICY IF EXISTS "System can update balances" ON professional_balances;
CREATE POLICY "System can update balances"
  ON professional_balances FOR UPDATE
  USING (true);
...
CREATE POLICY "Professionals can create own payouts"
...
CREATE POLICY "Users can view all reviews"
  ...
  USING (is_public = true OR reviewer_id = public.firebase_uid() OR reviewee_id = public.firebase_uid());
...
CREATE POLICY "Users can create reviews for own bookings"
  ...
  WITH CHECK (
    reviewer_id = public.firebase_uid()
    AND EXISTS (
      SELECT 1 FROM bookings 
...
CREATE POLICY "Reviewers can update own reviews"
  ...
  USING (reviewer_id = public.firebase_uid());
...
-- Chat rooms policies (metadata only, actual chat in Firestore)
...
-- Chat messages policies (metadata only, actual messages in Firestore)
...
CREATE POLICY "Users can view messages in own rooms"
  ...
  USING (
    EXISTS (
      ...
      AND (
        chat_rooms.customer_id = public.firebase_uid() 
        OR chat_rooms.professional_id = public.firebase_uid()
      )
    )
  );
...
CREATE POLICY "Admins can manage settings"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE users.firebase_uid = public.firebase_uid() 
