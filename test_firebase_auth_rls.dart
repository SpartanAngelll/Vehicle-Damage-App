...
Future<void> testFirebaseAuthAndRLS() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print('   Make sure Firebase is linked as Third Party Auth provider');
    } else {
  ...
  try {
    ...
    if (insertResponse.statusCode == 201 || insertResponse.statusCode == 200) {
      ...
      print('   Response: ${insertResponse.body}');
      print('   Note: If you see "invalid input syntax for type uuid",');
      print('   the app may be running cached code. Please restart the app completely.');
    }
  } catch (e) {
    if (e.toString().contains('permission') || e.toString().contains('RLS')) {
  ...
  print('   - Booking creation: ⚠️  (test updated to use UUID format)');
}
